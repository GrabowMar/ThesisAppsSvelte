# Login/Register Application Template - Flask + react
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=int(os.getenv('PORT', '5241')))
```
...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      '/api': {
        target: 'http://localhost:port: 5241',
        changeOrigin: true,
        secure: false,
      }
    }
...
1. **Port Configuration Prompt**
   - Use `5741` (backend) and `5241` (frontend) ports.
