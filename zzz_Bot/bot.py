...
class AssistantApp(tk.Tk):
    ...
    # -------------------------------------------------------------------------
    def _get_file_path(self, filename, section=None) -> Path:
        ...
        if section == "backend":
            if filename in ["app.py", "requirements.txt", "data.py", "utils.py", "Dockerfile"]:
                return Path('.') / model / app / 'backend' / filename
        elif section == "frontend":
            if filename in ["App.jsx", "App.css", "index.html"]:
                return Path('.') / model / app / 'frontend' / 'src' / filename
            elif filename in ["package.json", "vite.config.js", "Dockerfile"]:
                return Path('.') / model / app / 'frontend' / filename
        elif section == "docker":
            if filename == "docker-compose.yml":
                return Path('.') / model / app / filename
                
        # If path not determined by section rules
        if filename == "app.py":
            return Path('.') / model / app / 'backend' / filename
            ...
            return Path('.') / model / app / filename
        elif filename == "Dockerfile" and section == "backend":
            return Path('.') / model / app / 'backend' / filename
    ...
    def _create_new_file(self) -> None:
        ...
        filename = simpledialog.askstring("New File", 
                                        "Enter file name (including path if needed):")
        if not filename:
            return
            
        # Determine section based on file extension/name
        section = None
        if filename.endswith(".py"):
            section = "backend"
        elif filename.endswith((".jsx", ".css", ".html", ".js")):
            section = "frontend"
        ...
        # If path not determined, ask for section
        if not full_path:
            section = simpledialog.askstring("Section", 
                                           "Enter section (backend, frontend, or docker):")
            if section in ["backend", "frontend", "docker"]:
                self.current_section = section
                
                # Construct path based on section
                if section == "backend":
                    full_path = Path('.') / model / app / 'backend' / filename
                elif section == "frontend":
                    if filename.endswith((".jsx", ".css", ".html")):
                        full_path = Path('.') / model / app / 'frontend' / 'src' / filename
                    else:
                        full_path = Path('.') / model / app / 'frontend' / filename
                elif section == "docker":
                    full_path = Path('.') / model / app / filename
            else:
                self.status_var.set("Invalid section")
                return
    ...
    def _paste_to_file(self, filename, section=None) -> None:
        ...
        try:
            ...
            # Update editor and current file
            self.current_file_path = full_path
            self.current_file_var.set(f"Editing: {full_path}")
            self.summary_code_editor.delete("1.0", tk.END)
            self.summary_code_editor.insert(tk.END, content)
    ...
    def _copy_file_path(self, filename, section=None) -> None:
        ...
        self.clipboard_append(str(full_path))
    ...
    def _copy_current_path(self) -> None:
        ...
        self.clipboard_clear()
        self.clipboard_append(str(self.current_file_path))
        
        # Visual feedback
        self.status_var.set(f"✓ Copied path: {self.current_file_path}")
        self.after(1500, lambda: self.status_var.set("Ready"))
    ...
    def _update_app_status_display(self) -> None:
        ...
        # Get model directory and scan for apps
        ...
        for row in status_rows:
            ...
            if db_model == model:
                comments_map[app] = (comment, bool(app_py or app_react))
        ...
        for app_dir in app_folders:
            ...
            # If DB says generated but files don't exist, use DB value
    ...
    def _create_model_app_tab(self) -> None:
        ...
        ttk.Label(model_frame, text="Filter Model:").pack(side="left")
        self.model_app_model_var = tk.StringVar()
        models = ["All"] + [model.name for model in AI_MODELS]
        self.model_app_model_dropdown = ttk.Combobox(model_frame, textvariable=self.model_app_model_var,
    ...
    def _apply_model_app_filter(self, data):
        ...
        return [
            row for row in data
            if (ft in str(row["id"]).lower() or
                ft in row["model"].lower() or
                ft in row["app"].lower() or
                ft in row["comment"].lower())
        ]
    ...
    def _show_model_app_context_menu(self, event):
        ...
        menu.add_command(label=f"Load {model}/{app}", 
                        command=lambda: self._select_model_app(model, app))
        menu.add_separator()
        menu.add_command(label=f"Toggle app.py", 
                        command=lambda: self._toggle_app_py_status(item_id, row_id, model, app, app_py_disp, app_react_disp, comment))
        menu.add_command(label=f"Toggle App.jsx", 
                        command=lambda: self._toggle_app_jsx_status(item_id, row_id, model, app, app_py_disp, app_react_disp, comment))
        menu.add_separator()
        menu.add_command(label="Edit Comment", 
                        command=lambda: self._edit_model_app_comment(item_id, row_id, model, app, app_py_disp, app_react_disp, comment))
        
        menu.post(event.x_root, event.y_root)

    def _select_model_app(self, model, app):
    ...
    def _toggle_app_py_status(self, item_id, row_id, model, app, app_py_disp, app_react_disp, comment):
        """Toggle app.py status"""
        new_val = "☑" if app_py_disp == "☐" else "☐"
        values = [row_id, model, app, new_val, app_react_disp, comment]
    ...
    def _toggle_app_jsx_status(self, item_id, row_id, model, app, app_py_disp, app_react_disp, comment):
        """Toggle App.jsx status"""
        new_val = "☑" if app_react_disp == "☐" else "☐"
        values = [row_id, model, app, app_py_disp, new_val, comment]
    ...
    def _edit_model_app_comment(self, item_id, row_id, model, app, app_py_disp, app_react_disp, comment):
        """Edit comment for model/app"""
        new_comment = simpledialog.askstring("Edit Comment", "Enter comment:", initialvalue=comment)
        if new_comment is not None:
            values = [row_id, model, app, app_py_disp, app_react_disp, new_comment]
    ...
    def _create_research_tab(self) -> None:
        ...
        entry = ttk.Entry(filter_frame, textvariable=self.research_filter_var)
        entry.pack(side="left", fill="x", expand=True, padx=5)
        entry.bind("<KeyRelease>", lambda e: self._refresh_research_notes())
        ...
        self.research_table.column("model", width=80)
    ...
    def _refresh_research_notes(self) -> None:
        ...
        for note in notes:
            ...
            self.research_table.insert("", tk.END, values=(
                note_id, timestamp, model, app, note_type, summary
            ))
    ...
    def _refresh_web_prompts_list(self) -> None:
        ...
        for prompt in prompts:
            ...
            if filter_text and not (
                ...
                (response_text and filter_text in response_text.lower())
            ):
                continue
