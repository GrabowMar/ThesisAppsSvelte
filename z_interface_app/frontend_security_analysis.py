...
class FrontendSecurityAnalyzer:
    ...
    def __init__(self, base_path: Union[str, Path]):
        ...
        self.available_tools = {
            ...
            "eslint": bool(get_executable_path("npx")),
    ...
    def _determine_tool_status(self, tool_name: str, issues: List[SecurityIssue], output: str) -> str:
        ...
        if "error" in output_lower or "failed" in output_lower:
            if not issues and "found 0 vulnerabilities" not in output_lower:
                return ToolStatus.ERROR
            else:
                return ToolStatus.ISSUES_WITH_ERRORS.format(count=len(issues))

        if issues:
            return ToolStatus.ISSUES_FOUND.format(count=len(issues))
    ...
    def _parse_npm_audit(self, stdout: str) -> List[SecurityIssue]:
        ...
        for key, vuln_info in vulnerabilities.items():
            ...
            if isinstance(fix_available, dict):
                ...
                if fix_ver:
                    fix_text = f"Update {name} to version {fix_ver}"
    ...
    def _run_tool_with_setup(
        ...
        status = {tool_name: ToolStatus.NOT_RUN}
        raw_output = ""
        
        try:
            # Get tool-specific setup from the provided function
            args, config_dict, input_data = setup_func(app_path)
            if not args:
                # Setup indicated tool should be skipped
    ...
    def _setup_npm_audit(self, app_path: Path) -> Tuple[List[str], Optional[Dict], Optional[str]]:
        """Set up npm audit analysis."""
        package_json_path = app_path / "package.json"
        package_lock_path = app_path / "package-lock.json"

        if not package_json_path.exists():
            logger.warning(f"No package.json found in {app_path}, skipping npm-audit.")
            return [], {"status": "❌ No package.json"}, f"No package.json found in {app_path}, skipping npm-audit."
        ...
        if not package_lock_path.exists():
            ...
            issues, init_output = self._run_frontend_tool(
    ...
    def _setup_eslint(self, app_path: Path) -> Tuple[List[str], Optional[Dict], Optional[str]]:
        """Set up ESLint analysis."""
        has_files, _ = self._check_source_files(app_path)
        if not has_files:
            logger.warning(f"No relevant frontend files found in {app_path}, skipping eslint.")
            return [], {"status": "❌ No files to analyze"}, f"No relevant frontend files found in {app_path}, skipping eslint."
        ...
        if project_config_exists:
            ...
            # No project config exists - note we'll handle the temp config creation in run_security_analysis
            logger.info("No project ESLint config found, using a basic config will be created during execution.")
    ...
    def _setup_jshint(self, app_path: Path) -> Tuple[List[str], Optional[Dict], Optional[str]]:
        """Set up JSHint analysis."""
        has_files, source_files = self._check_source_files(app_path)
        if not has_files:
            logger.warning(f"No frontend files found in {app_path}, skipping jshint.")
            return [], {"status": "❌ No files to analyze"}, f"No frontend files found in {app_path}, skipping jshint."

        js_files = [f for f in source_files if f.endswith(('.js', '.jsx'))]
        if not js_files:
            logger.warning(f"No JavaScript/JSX files found in {app_path}, skipping jshint.")
            return [], {"status": "❌ No JS files to analyze"}, f"No JavaScript/JSX files found in {app_path}, skipping jshint."
        ...
        # Check for project JSHint config
    ...
    def _setup_snyk(self, app_path: Path) -> Tuple[List[str], Optional[Dict], Optional[str]]:
        """Set up Snyk analysis."""
        if not (app_path / "package.json").exists():
            logger.warning(f"No package.json found in {app_path}, skipping snyk.")
            return [], {"status": "❌ No package.json"}, f"No package.json found in {app_path}, skipping snyk."
            
        # We'll handle the temp file creation in run_security_analysis
    ...
    def _run_eslint(self, app_path: Path) -> Tuple[List[SecurityIssue], Dict[str, str], str]:
        ...
        # First check if we can run ESLint
        args, config_dict, _ = self._setup_eslint(app_path)
        if not args:
            # Setup indicated tool should be skipped
            return [], status, raw_output
            
        # Check if we need to create a temp config
        if config_dict and config_dict.get("needs_temp_config"):
            ...
            try:
                with self._create_temp_config("eslint_config_", eslint_config, ".eslintrc.json") as temp_config_path:
                    # Add config and scan dir to arguments
                    complete_args = args + ["--config", str(temp_config_path), scan_dir]
                    
                    # Run ESLint with temp config
                    issues, run_output = self._run_frontend_tool(
                        tool_name, complete_args[0], complete_args[1:], 
                        app_path, parser=self._parse_eslint
                    )
            ...
            # Run ESLint with existing config
            issues, run_output = self._run_frontend_tool(
        ...
        # Check for plugin errors that might not be reflected in the return code
    ...
    def _run_jshint(self, app_path: Path) -> Tuple[List[SecurityIssue], Dict[str, str], str]:
        """Run JSHint and parse results."""
        tool_name = "jshint"
        status = {tool_name: ToolStatus.NOT_RUN}
        raw_output = ""
        issues = []
        
        # First check if we can run JSHint
        ...
        # Check if we need to create a temp config
        if config_dict and config_dict.get("needs_temp_config"):
            files_to_scan = config_dict.get("files_to_scan", [])
            
            # Create a basic JSHint config
            jshint_config = {
                "esversion": 9, 
                "browser": True, 
                "node": True,
            ...
            try:
                with self._create_temp_config("jshint_config_", jshint_config, ".jshintrc") as temp_config_path:
                    # Add config and files to arguments
                    complete_args = args + ["--config", str(temp_config_path)] + files_to_scan
                    
                    # Run JSHint with temp config
                    issues, run_output = self._run_frontend_tool(
                        tool_name, complete_args[0], complete_args[1:], 
                        app_path, parser=self._parse_jshint
                    )
                    raw_output = run_output
            except Exception as exc:
                status[tool_name] = f"❌ Failed: {exc}"
            ...
            # Run JSHint with existing config
            issues, run_output = self._run_frontend_tool(
    ...
    def _run_snyk(self, app_path: Path) -> Tuple[List[SecurityIssue], Dict[str, str], str]:
        ...
        # First check if we can run Snyk
        args, config_dict, _ = self._setup_snyk(app_path)
        if not args:
            # Setup indicated tool should be skipped
            return [], status, raw_output
        
        # Snyk requires a temporary output file for JSON results
        try:
    ...
    def run_security_analysis(
        ...
        if not runnable_tools:
            ...
            for tool in self.all_tools:
                if tool not in tools_to_attempt:
                    final_status[tool] = ToolStatus.SKIPPED
                elif not self.available_tools.get(tool):
                    final_status[tool] = "❌ Not Available"
                else:
                    final_status[tool] = ToolStatus.UNKNOWN
        ...
        # Use a thread pool to run the tools in parallel
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(len(runnable_tools), 4)) as executor:
            future_to_tool = {
                ...
                for tool in runnable_tools if tool in tool_map
            }
        ...
        for tool in self.all_tools:
            if tool not in tool_status:
                ...
                elif tool not in runnable_tools:
                    tool_status[tool] = ToolStatus.SKIPPED
                    tool_outputs[tool] = f"{tool} was available but not selected for this run."
                else:
                    tool_status[tool] = ToolStatus.UNKNOWN
                    tool_outputs[tool] = "Tool status was not recorded."

        # Sort issues by severity, confidence, etc.
