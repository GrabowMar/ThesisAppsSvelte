```markdown
# File Uploader Application Template - Flask + react
...
A file handling application built with Flask and react, featuring upload and download capabilities.
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=int(os.getenv('PORT', '5333')))
```
...
export default defineConfig({
  ...
  server: {
    ...
    proxy: {
      '/api': {
        target: 'http://localhost:port: 5333',
        changeOrigin: true,
        secure: false,
      }
    }
...
1. **Port Configuration Prompt**
   - Use `5833` (backend) and `5333` (frontend) ports.
